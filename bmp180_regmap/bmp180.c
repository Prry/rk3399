...
{
	return regmap_bulk_read(pdev->bm_regmap, reg, pdata, size);
}

static int bmp180_write_regs(struct bmp180_dev *pdev, uint8_t reg, uint8_t *pdata, int size)
{
	return regmap_bulk_write(pdev->bm_regmap, reg, pdata, size);
}
...
{
	return regmap_write(pdev->bm_regmap, reg, (int)data);
}
...
{     
	...
	bmp180_device.bm_regmap = regmap_init_i2c(client, &bmp180_regmap_config);
